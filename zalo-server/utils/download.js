...
function filenameFromResponse(url, response) {
  ...
  if (utf8Match?.[1]) {
    try { raw = decodeURIComponent(raw); } catch { /* keep undecoded input */ }
  }
...
export async function downloadToTemp(url, {
  ...
  try {
    ...
    const safeName = safeFilename(filename || filenameFromResponse(parsed, response));
