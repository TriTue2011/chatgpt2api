    ...
    return (
      <div className="flex h-full min-h-[260px] items-center justify-center text-center sm:min-h-[420px]">
        <div className="w-full max-w-4xl">
          <h1
            className="text-2xl font-semibold tracking-tight text-[var(--foreground)] sm:text-3xl md:text-5xl"
            style={{
          ...
          <p
            className="mx-auto mt-3 max-w-[280px] text-sm italic tracking-[0.01em] text-[var(--muted-foreground)] sm:mt-4 sm:max-w-none sm:text-[15px]"
            style={{
              fontFamily: '"Palatino Linotype","Book Antiqua","URW Palladio L","Times New Roman",serif',
            }}
          >
            Lưu trữ lịch sử và trạng thái tác vụ cục bộ trong cùng một cửa sổ, đồng thời tiếp tục các chỉnh sửa mới từ kết quả đã có.
          </p>
        </div>
      </div>
    );
          ...
          <div key={turn.id} className="flex flex-col gap-3 sm:gap-4">
            {!turn.promptDeleted ? (
              <div className="flex justify-end">
                <div className="max-w-[90%] px-1 py-1 text-[14px] leading-6 text-[var(--foreground)] sm:max-w-[82%] sm:text-[15px] sm:leading-7">
                  <div className="mb-1.5 flex flex-wrap justify-end gap-2 text-[11px] text-[var(--muted-foreground)] sm:mb-2">
                  ...
                  <div className="mt-2 flex flex-wrap justify-end gap-1.5">
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded-full bg-[var(--secondary)] px-2.5 py-1 text-[11px] font-medium text-[var(--muted-foreground)] transition hover:bg-[var(--secondary)] hover:text-[var(--foreground)]"
                    >
                      Dùng lại cấu hình
                    </button>
                    <button
                      ...
                      className="inline-flex size-6 items-center justify-center rounded-full text-[var(--foreground)] transition hover:bg-rose-50 hover:text-rose-500"
                      aria-label="Xóa nhật ký prompt"
                    >
                ...
                <div className="w-full p-1">
                  {turn.referenceImages.length > 0 ? (
                    <div className="mb-4 flex flex-col items-end">
                      <div className="mb-3 text-xs font-medium text-[var(--muted-foreground)]">Ảnh tham chiếu lượt này</div>
                        ...
                        {turn.referenceImages.map((image, index) => (
                          <div key={`${turn.id}-${image.name}-${index}`} className="flex flex-col items-end gap-2">
                            <button
                              ...
                              className="group relative h-24 w-24 overflow-hidden border border-[var(--border)]/80 bg-[var(--secondary)]/60 text-left transition hover:border-[var(--border)]"
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              className="rounded-full border-[var(--border)] bg-[var(--card)] text-[var(--foreground)] hover:brightness-110"
                  ...
                  <div className="mb-3 flex flex-wrap items-center gap-1.5 text-[11px] text-[var(--muted-foreground)] sm:mb-4 sm:gap-2 sm:text-xs">
                    <span className="rounded-full bg-[var(--secondary)] px-3 py-1">{turn.count} ảnh</span>
                    <span className="rounded-full bg-[var(--secondary)] px-3 py-1">{getTurnStatusLabel(turn.status)}</span>
                    {turn.status === "queued" ? (
                      <span className="rounded-full bg-amber-50 px-3 py-1 text-amber-700">Đang chờ hoàn thành các tác vụ trước đó trong hội thoại</span>
                    ) : null}
                  </div>
                    ...
                    {turn.images.map((image, index) => {
                          ...
                          >
                            ...
                            <div className="flex flex-col gap-1 px-0.5 py-1 text-[10px] sm:flex-row sm:items-center sm:justify-between sm:gap-2 sm:px-3 sm:py-3 sm:text-xs">
                              <div className="min-w-0 text-[var(--muted-foreground)]">
                                ...
                                {imageMeta ? <span className="block text-[var(--muted-foreground)] sm:ml-2 sm:inline">{imageMeta}</span> : null}
                              </div>
                              <div className="flex items-center gap-1.5">
                                <Button
                                  variant="outline"
                                  size="sm"
                                  className="h-7 w-7 rounded-full border-[var(--border)] bg-[var(--card)] px-0 text-[10px] text-[var(--foreground)] hover:brightness-110 sm:h-8 sm:w-fit sm:px-3 sm:text-xs"
                                  onClick={() => onContinueEdit(selectedConversation.id, image)}
                          ...
                          >
                            <div className="flex h-full min-h-16 flex-col items-center justify-center gap-1.5 px-2 py-2 text-center text-[11px] leading-4 text-rose-600 sm:gap-3 sm:px-6 sm:py-8 sm:text-sm sm:leading-6">
                              ...
                              <button
                                ...
                                className="rounded-full bg-[var(--card)] px-2 py-1 text-[10px] font-medium text-rose-600 shadow-sm transition hover:bg-rose-100 sm:px-3 sm:text-xs"
                              >
                                Thử lại ảnh này
                              </button>
                            </div>
                      ...
                      return (
                        <div
                          key={image.id}
                          className={cn(
                            "break-inside-avoid overflow-hidden rounded-xl border border-[var(--border)]/80 bg-[var(--secondary)]/80 sm:rounded-none",
                        ...
                        >
                          <div className="flex h-full flex-col items-center justify-center gap-1.5 px-2 py-3 text-center text-[var(--muted-foreground)] sm:gap-3 sm:px-6 sm:py-8">
                            <div className="rounded-full bg-[var(--card)] p-2 shadow-sm sm:p-3">
                              {turn.status === "queued" ? (
                  ...
                  <div className="mt-3 flex items-center gap-1.5 text-[11px] sm:mt-4">
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded-full bg-[var(--secondary)] px-2.5 py-1 font-medium text-[var(--muted-foreground)] transition hover:bg-[var(--secondary)] hover:text-[var(--foreground)]"
                    >
