...
import { MemoryLinksCard, LuuTruLinksCard } from "./memory-links-card";
...
function parseAdminEntries(b: any): AdminEntry[] {
  ...
  const push = (e: Partial<AdminEntry> & { chat_id?: string }) => {
    ...
    out.push(emptyAdmin({
      ...
      markdown_color: String(
        e.markdown_color || (e as any).emphasis_color || "orange",
      ).trim().toLowerCase() || "orange",
      markdown_size: String(
        e.markdown_size || (e as any).emphasis_size || (e as any).text_size || "normal",
        ...
        return (
          <div key={row.id} className="rounded-md border border-border p-2 space-y-2">
            ...
            {/* Admin #N — CHỈ nơi NHẬN thông báo (notify-only). Model / persona /
                ...
                Collapse nhiều cấp: mặc định ẩn, hiện tên + ID; kích để mở. */}
            <div className="space-y-2">
              <label className="text-xs font-medium text-muted-foreground">
                Admin #1, #2… — nơi NHẬN thông báo (🔔 lỗi · 📋 log · 🔄 cập nhật · 💬 chat mới).
                Muốn admin chat / ra lệnh: thêm thread admin vào tab «Lọc thread».
              </label>
              {row.admins.map((a, idx) => {
                const akey = `${row.id}-${idx}`;
                const open = openAdmin[akey] ?? !a.chat_id.trim();
                const title = a.name.trim() || (a.chat_id.trim() ? "(chưa đặt tên)" : "Admin mới");
                return (
                <div key={idx} className="rounded-md border border-border bg-muted/10">
                  <div className="flex items-center gap-2 p-2 cursor-pointer select-none"
                    ...
                    <Button type="button" variant="ghost" size="sm" className="h-7 px-2 text-[10px]"
                      onClick={(e) => { e.stopPropagation(); removeAdmin(row.id, idx); }}>Xóa</Button>
                  </div>
                  {open ? (
...
export function TelegramCloudflareCard() {
  ...
  const [resolving, setResolving] = useState<string>("");
  // Collapse nhiều cấp Zalo Cá Nhân: acc (key ownId) → admin (key ownId-idx).
  const [openZpAcc, setOpenZpAcc] = useState<Record<string, boolean>>({});
  const [openZpAdmin, setOpenZpAdmin] = useState<Record<string, boolean>>({});
  ...
  type TopicRow = {
    ...
    /** Bot trả lời trong nhóm thì tag người vừa hỏi (config tag_asker, mặc định tắt) */
    tagAsker: boolean;
    /** Chỉ người trong danh sách mới được giao tiếp (config thread_user_only) */
    userOnly: boolean;
    /** Tắt HẲN ChatGPT ở thread này (chuyển tiếp + nhật ký vẫn chạy) */
    aiOff: boolean;
    forward: boolean; forwardUrl: string; forwardTagOnly: boolean; forwardKeyword: string;
  ...
  type FilterRow = {
    ...
    /** Chỉ chuyển webhook khi tin có tag — không tag thì ChatGPT trả lời */
    forwardTagOnly: boolean;
    /** Tag RIÊNG của chuyển tiếp; rỗng = dùng chung từ khóa «bắt buộc tag» */
  ...
  useEffect(() => {
    ...
    const tmf = (config as any)?.thread_mention_filters as Record<string, { required?: boolean; keyword?: string; reply_to_self?: boolean; self_keyword?: string; ai_off?: boolean; tag_asker?: boolean }> | undefined;
    const tuo = (config as any)?.thread_user_only as Record<string, boolean> | undefined;
    const tff = (config as any)?.thread_forward_filters as Record<string, { enabled?: boolean; url?: string; tag_mode?: boolean; keyword?: string }> | undefined;
    ...
    const rows: FilterRow[] = Object.entries(tf || {})
      ...
      .map(([key, groups]) => {
        ...
        return {
          ...
          requireMention: false, mentionKeyword: "", replyToSelf: false, selfKeyword: "", tagAsker: false, userOnly: false, aiOff: false, forward: false, forwardUrl: "", forwardTagOnly: false, forwardKeyword: "",
          aiModel: modelOf(key),
          haFastpath: fpOf(key),
          topics: [],
        };
    ...
    const ensureTopic = (key: string) => {
      ...
      if (!t) {
        ...
        t = {
          ...
          users: [], requireMention: false, mentionKeyword: "", replyToSelf: false, selfKeyword: "", tagAsker: false, userOnly: false, aiOff: false,
          forward: false, forwardUrl: "", forwardTagOnly: false, forwardKeyword: "",
    ...
    for (const [key, v] of Object.entries(tmf || {})) {
      ...
      target.aiOff = !!v?.ai_off;
      target.tagAsker = !!v?.tag_asker;
    }
    ...
    for (const [key, v] of Object.entries(tff || {})) {
      ...
      if (!got.userId) {
        ...
        holder.forwardTagOnly = !!v?.tag_mode;
        holder.forwardKeyword = String(v?.keyword || "");
        continue;
      }
      let u = holder.users.find((x) => x.userId === got.userId);
      if (!u) {
        u = { id: rowSeq.current++, userId: got.userId, groups: [...holder.groups], forward: false, forwardUrl: "", forwardTagOnly: false, forwardKeyword: "" };
        holder.users.push(u);
      }
      u.forward = !!v?.enabled;
      u.forwardUrl = String(v?.url || "");
      u.forwardTagOnly = !!v?.tag_mode;
      u.forwardKeyword = String(v?.keyword || "");
  ...
  const commitFilters = (rows: FilterRow[]) => {
    ...
    const tmf: Record<string, { required: boolean; keyword: string; reply_to_self?: boolean; self_keyword?: string; ai_off?: boolean; tag_asker?: boolean }> = {};
    const tuo: Record<string, boolean> = {};
    const tff: Record<string, { enabled: boolean; url: string; tag_mode: boolean; keyword: string }> = {};
    ...
    for (const r of rows) {
      ...
      if (r.forward || rUrl)
        tff[parent] = { enabled: r.forward && !!rUrl, url: rUrl,
                        tag_mode: r.forwardTagOnly, keyword: r.forwardKeyword.trim() };
      // Thread CÁ NHÂN: không lưu tag/lọc user (không áp dụng cho chat 1-1).
      if (r.kind !== "user") {
        if (r.requireMention || r.mentionKeyword.trim() || r.replyToSelf || r.selfKeyword.trim() || r.aiOff || r.tagAsker)
          tmf[parent] = { required: r.requireMention, keyword: r.mentionKeyword.trim(), reply_to_self: r.replyToSelf, self_keyword: r.selfKeyword.trim(), ai_off: r.aiOff, tag_asker: r.tagAsker };
        ...
        for (const t of r.topics) {
          ...
          if (t.requireMention || t.mentionKeyword.trim() || t.replyToSelf || t.selfKeyword.trim() || t.aiOff || t.tagAsker)
            tmf[tkey] = { required: t.requireMention, keyword: t.mentionKeyword.trim(), reply_to_self: t.replyToSelf, self_keyword: t.selfKeyword.trim(), ai_off: t.aiOff, tag_asker: t.tagAsker };
          if (t.userOnly) tuo[tkey] = true;
          const tUrl = t.forwardUrl.trim();
          if (t.forward || tUrl)
            tff[tkey] = { enabled: t.forward && !!tUrl, url: tUrl,
                          tag_mode: t.forwardTagOnly, keyword: t.forwardKeyword.trim() };
      ...
      r.id === rowId
        ? {
          ...
          topics: [...r.topics, {
            ...
            mentionKeyword: r.mentionKeyword, replyToSelf: r.replyToSelf, selfKeyword: r.selfKeyword, tagAsker: r.tagAsker, userOnly: r.userOnly, aiOff: r.aiOff,
            forward: false, forwardUrl: "", forwardTagOnly: false, forwardKeyword: "", aiModel: "", haFastpath: "",
          }],
        }
        : r));
  ...
  const addUserRow = (rowId: number, topicId: number | null = null) => {
    ...
    mapUsers(rowId, topicId, (us) => [...us, {
      ...
      forward: !!h?.forward, forwardUrl: "", forwardTagOnly: false, forwardKeyword: "",
    }]);
  };
  ...
  useEffect(() => {
    ...
    if (subTab === "directory" && chTab !== "memlinks" && chTab !== "kholinks"
        && chTab !== "nhatky")
      void loadDirectory();
      ...
      <CardContent className="space-y-4 pt-4">
        ...
        <div className="flex flex-wrap gap-2">
          {([["tg", "📨 Telegram"], ["zalo", "💬 Zalo Bot"], ["zalop", "👤 Zalo Cá Nhân"],
             ["memlinks", "🔗 Kết nối bộ nhớ"], ["kholinks", "☁️ Kết nối kho đám mây"],
             ["nhatky", "🗒️ Nhật ký nhóm"]] as const)
            .map(([k, lb]) => (
        ...
        {chTab !== "memlinks" && chTab !== "kholinks" && chTab !== "nhatky" && (
        <div className="flex flex-wrap gap-2">
          {(([
        ...
        {/* ── Kết nối kho đám mây — SỔ RIÊNG, không ăn theo kết nối bộ nhớ ── */}
        {chTab === "kholinks" && <LuuTruLinksCard />}
        ...
        {chTab === "tg" && subTab === "settings" && (
          ...
          <p className="text-[10px] text-muted-foreground">
            <b>Admin</b> ở thẻ bot chỉ chọn <b>thông báo</b> nhận về (🔔 lỗi · 📋 log · 🔄 cập nhật · 💬 chat mới).
        ...
        {chTab === "zalo" && subTab === "settings" && (
          ...
          <p className="text-[10px] text-muted-foreground">
            <b>Admin</b> ở thẻ bot chỉ chọn <b>thông báo</b> nhận về (🔔 · 📋 · 🔄 · 💬 chat mới).
            Model / chức năng / persona / giọng nói cài ở tab <b>«Lọc thread»</b>.
            Admin muốn chat / ra lệnh: thêm thread admin vào Lọc thread — không có thì bot <b>im lặng</b> (chỉ nhận log).
          </p>
        ...
        {/* Zalo Cá Nhân: tab «Cài đặt kênh» đã BỎ — bật/tắt kênh, bật AI, tài
            khoản gửi mặc định, bật/tắt + fallback từng tài khoản, hoạt động &
            blacklist đều nằm trong «🔑 Tài khoản & QR» → tab con «Cài đặt kênh».
            Model AI và đường tắt HA cài theo từng thread ở «Lọc thread». */}
        ...
        {subTab === "filter" && (
          ...
          {filterRows.filter((r) => r.botKey === chTab || r.botKey.startsWith(`${chTab}:`)).map((row) => {
            ...
            const uCount = row.users.filter((u) => u.userId.trim()).length;
            return (
            <div key={row.id} className="rounded-md border border-border p-2 space-y-2">
              ...
              <div className="flex items-center gap-2 cursor-pointer select-none"
                ...
                <span className="text-[10px] text-muted-foreground shrink-0">
                  {row.kind === "user" ? "Cá nhân" : `Nhóm${uCount ? ` · ${uCount} user` : ""}`}
                </span>
                <Button type="button" variant="ghost" size="sm" className="h-6 px-2 text-[10px]"
                  onClick={(e) => { e.stopPropagation(); removeFilterRow(row.id); }}>Xóa</Button>
              </div>
              {fOpen ? (
              <>
              <div className="flex items-center gap-2 flex-wrap">
              ...
              {/* TẮT HẲN ChatGPT — ngoài khối "chỉ hiện với nhóm" bên dưới, vì
                  chat 1-1 cũng phải tắt được. Chuyển tiếp và nhật ký vẫn chạy. */}
              ...
              <div className="rounded border border-dashed border-border/70 p-2 space-y-1.5">
                ...
                {/* Chủ máy 15/09/2026: bật/tắt tag người hỏi, giống ô «bắt buộc tag»; mặc định tắt.
                    Chỉ Zalo Cá Nhân (zca-js) gửi được mention theo người. */}
                {row.botKey.startsWith("zalop") && (
                  <label className="flex items-center gap-1.5 text-xs cursor-pointer select-none">
                    <input
                      type="checkbox"
                      className="relative before:absolute before:left-1/2 before:top-1/2 before:size-6 before:-translate-x-1/2 before:-translate-y-1/2 before:content-[''] size-3.5"
                      checked={row.tagAsker}
                      onChange={() => setFilterField(row.id, { tagAsker: !row.tagAsker })}
                    />
                ...
                {row.forward && (
                  <>
                    ...
                    <label className="flex items-center gap-1.5 text-xs cursor-pointer select-none">
                      ...
                      />
                      🏷️ Chỉ chuyển khi tin có TAG — tin thường thì ChatGPT trả lời
                    </label>
                    {row.forwardTagOnly && (
                      <Input
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      {row.forwardTagOnly ? (
                        <>
                          Tin chứa tag trên → <b>chỉ đi webhook</b>, ChatGPT im lặng. Tin không có
                          tag → ChatGPT trả lời như thường, không chuyển. Áp cho <b>cả tin của
                          người khác lẫn tin bạn tự gõ</b>. Đặt tag riêng ở đây để <code>@bot</code>
                          vẫn gọi được AI còn <code>@n8n</code> thì đẩy sang webhook — hai việc
                          ngược nhau nên đừng dùng chung một từ khóa.
                        </>
                      ) : (
                        <>
