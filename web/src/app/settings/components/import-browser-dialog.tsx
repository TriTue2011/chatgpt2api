      ...
      <DialogContent showCloseButton={false} className="max-h-[90vh] max-w-5xl rounded-2xl p-6">
        ...
        <div className="rounded-xl border border-[var(--border)]">
          ...
          <div className="max-h-[420px] overflow-auto">
            {pagedFiles.length === 0 ? (
              <div className="flex items-center justify-center py-12 text-sm text-[var(--muted-foreground)]">Không có tài khoản từ xa phù hợp</div>
            ) : (
              <div className="divide-y divide-[var(--border)]">
                {pagedFiles.map((item) => (
                  <label key={item.name} className="flex cursor-pointer items-center gap-3 px-4 py-3 hover:brightness-110">
                    <Checkbox
                      checked={selectedNames.includes(item.name)}
                      onCheckedChange={(checked) => toggleFile(item.name, Boolean(checked))}
                    />
                    <div className="min-w-0">
                      <div className="truncate text-sm font-medium text-[var(--foreground)]">{item.email || item.name}</div>
                      <div className="truncate text-xs text-[var(--muted-foreground)]">{item.name}</div>
                    </div>
                  </label>
                ))}
              </div>
