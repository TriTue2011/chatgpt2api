...
const KHO: Record<string, {
  ...
  drive: {
    cacBuoc: [
      "TẠO DỰ ÁN — vào console.cloud.google.com, chọn dự án có sẵn hoặc bấm New Project để tạo mới.",
      "BẬT API — menu trái: APIs & Services → Library → gõ «Google Drive API» → bấm Enable. Nếu nút ghi «Manage» tức là đã bật rồi, bỏ qua.",
      "KHAI MÀN HÌNH XIN QUYỀN — APIs & Services → OAuth consent screen (giao diện mới: Google Auth Platform → Branding). Chọn External, điền tên ứng dụng và email của mình, rồi Save.",
      "CHUYỂN SANG CHẠY THẬT — vẫn ở màn hình đó, tìm nút PUBLISH APP và bấm. BẮT BUỘC: để ở chế độ Testing thì Google chặn đăng nhập với lỗi «403 access_denied», và kể cả thêm mình vào danh sách người kiểm thử thì quyền cũng tự hết sau 7 ngày.",
      "Google có thể mời xác minh ứng dụng — BỎ QUA. Xác minh chỉ cần khi ứng dụng phục vụ người ngoài; đây là ứng dụng riêng, chỉ mình mình dùng.",
      "TẠO MÃ — APIs & Services → Credentials → + Create Credentials → OAuth client ID → Application type chọn «Desktop app» → Create. Cửa sổ hiện ra có Client ID và Client Secret (dạng GOCSPX-…), sao chép cả hai.",
      "ĐIỀN VÀO ĐÂY — đặt tên kho ở ô trên cùng, dán Client ID và Client Secret vào hai ô tương ứng. Ô Token để trống.",
      "BẤM «Đăng nhập Google» — tab mới mở ra, đăng nhập tài khoản có Drive muốn dùng.",
      "Gặp cảnh báo «Google hasn't verified this app» thì bấm Advanced → Go to … (unsafe) → Continue → Allow. Chữ «unsafe» chỉ vì ta đã bỏ qua bước xác minh ở trên; ứng dụng do chính mình tạo.",
      "SAU KHI CHO PHÉP, trình duyệt sẽ báo LỖI KHÔNG KẾT NỐI ĐƯỢC — đúng như vậy, không phải hỏng. Nhìn lên thanh địa chỉ, sao chép TOÀN BỘ dòng dạng http://127.0.0.1:53682/?state=…&code=… rồi dán vào ô hiện ra bên dưới.",
      "Bấm «Hoàn tất và tạo kho». Xong — bấm «Thử» ở danh sách kho phía trên để xác nhận kết nối.",
      "Muốn bot chỉ dùng MỘT thư mục thay vì cả Drive: mở thư mục đó trên Drive, thanh địa chỉ có dạng …/folders/<mã>, dán phần mã vào ô «Mã thư mục gốc» trước khi đăng nhập.",
    ],
    nhan: "Google Drive",
    loai: "drive",
    ...
    truong: [
      ...
      { khoa: "team_drive", nhan: "Mã ổ chung (Shared Drive)",
        chuThich: "CHỈ Google Workspace mới có. Dùng đường tài khoản dịch vụ thì bắt buộc điền ô này." },
      { khoa: "service_account_file", nhan: "Tệp khoá JSON (chỉ cho Shared Drive)", taiTep: true,
        chuThich: "Chỉ dùng khi có ổ chung Workspace. Với Drive cá nhân thì bỏ trống và dùng ô Token." },
      { khoa: "impersonate", nhan: "Mạo danh người dùng (Workspace)",
        goiY: "nguoidung@tenmien.com",
        chuThich: "Chỉ cho Google Workspace có uỷ quyền toàn miền." },
    ],
  },
  googlephotos: {
    cacBuoc: [
