    ...
    <section className="space-y-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="space-y-1">
          <div className="text-xs font-semibold tracking-[0.18em] text-[var(--muted-foreground)] uppercase">Logs</div>
          <h1 className="text-2xl font-semibold tracking-tight">Quản lý nhật ký</h1>
        </div>
        <div className="flex flex-wrap gap-2">
          <Select value={type} onValueChange={setType}>
            <SelectTrigger className="h-10 w-[150px] rounded-xl border-[var(--border)] bg-[var(--card)]"><SelectValue /></SelectTrigger>
            <SelectContent>
          ...
          <Button onClick={() => void loadLogs()} disabled={isLoading} className="h-10 rounded-xl bg-[var(--primary)] px-4 text-[var(--primary-foreground)] hover:brightness-110">
      ...
      <Card className="overflow-hidden rounded-2xl border-[var(--border)] bg-[var(--card)]/90 shadow-sm">
        <CardContent className="p-0">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--border)] px-5 py-4">
            ...
            <div className="flex items-center gap-2">
              <Button variant="ghost" className="h-8 rounded-lg px-3 text-[var(--muted-foreground)]" onClick={() => void loadLogs()} disabled={isLoading}>
            ...
            <Table className="min-w-[900px]">
              ...
              <TableBody>
                {currentRows.map((item) => {
                  ...
                  return (
                    <TableRow key={item.id} className="text-[var(--muted-foreground)]">
                      <TableCell>
                            ...
                            <div className="flex items-center gap-1.5">
                              {urls.slice(0, 3).map((url, imageIndex) => (
                                <button
                                  key={`${url}-${imageIndex}`}
                                  type="button"
                                  className="relative size-9 overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--secondary)]"
                              ...
                              {urls.length > 3 ? <span className="text-xs text-[var(--muted-foreground)]">+{urls.length - 3}</span> : null}
                            </div>
                          ) : (
          ...
          {!isLoading && items.length === 0 ? <div className="px-6 py-14 text-center text-sm text-[var(--muted-foreground)]">Không tìm thấy nhật ký</div> : null}
        </CardContent>
      </Card>
        ...
        <DialogContent className="flex h-[min(88vh,860px)] w-[min(92vw,920px)] flex-col overflow-hidden rounded-2xl p-0">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5">
            <div className="space-y-4">
              <div className="grid gap-3 rounded-xl border border-[var(--border)] bg-[var(--card)] p-4 text-sm text-[var(--muted-foreground)] md:grid-cols-2">
                ...
                <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
                  {detailUrls.map((url, index) => (
                    <button
                      key={url}
                      type="button"
                      className="aspect-square overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--secondary)]"
                      onClick={() => {
                        setLightboxIndex(index);
                        setLightboxOpen(true);
                      }}
              ...
              <pre className="max-h-[72vh] overflow-auto rounded-xl border border-[var(--border)] bg-[var(--secondary)] p-4 text-xs leading-6 text-[var(--foreground)]">
