...
export default function VideoManagerPage() {
  ...
  const loadVideos = async () => {
    try {
      // Load all media files: video + audio
      const resp = await request.get("/api/images?media_type=all");
      ...
      toast.error("Không tải được danh sách media");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20">
          <LoaderCircle className="size-8 animate-spin text-[var(--muted-foreground)]" />
        </div>
      ) : videos.length === 0 ? (
        <Card className="rounded-2xl card-3d card-tint-sky">
          <CardContent className="flex flex-col items-center py-16 text-center">
            <Film className="size-16 text-[var(--secondary-foreground)]" />
            <p className="mt-4 text-[var(--muted-foreground)]">Chưa có video nào</p>
            <p className="text-sm text-[var(--muted-foreground)]">Video tạo từ Veo sẽ hiển thị ở đây</p>
          </CardContent>
        </Card>
      ) : (
          ...
          {videos.map((video) => (
            <Card key={video.path} className="group rounded-2xl card-3d card-tint-sky hover:shadow-md transition-all">
              <CardContent className="p-4 flex flex-col gap-3">
                <div className="relative aspect-video rounded-lg overflow-hidden bg-black/5 border border-black/10">
                  <video 
                    src={`/images/${video.path}`} 
                    controls 
                    preload="metadata"
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-[var(--foreground)] truncate" title={video.name}>{video.name}</p>
                  <p className="text-xs text-[var(--muted-foreground)]">{formatSize(video.size_bytes)}</p>
                </div>
