...
def _kenh_zalop(thread_id: str, thread_type: int) -> "object":
    """Bốn cửa gửi ra của Zalo cá nhân, gói cho ``services.video_giao``.

    Toàn bộ nghiệp vụ (menu bảy ô, tải hai bản, ghép chữ, lồng tiếng, bốn ô đọc
    hiểu) nằm ở module chung để Telegram dùng ĐÚNG một bản; chỗ này chỉ còn
    phần riêng của Zalo: gửi tin có màu, và câu trả lời dài thì đóng .docx vì
    Zalo cắt tin ở 2.000 ký tự.
    """
    from services import video_giao as _vg

    return _vg.Kenh(
        gui_tin=lambda chu: send_message(thread_id, chu, thread_type),
        gui_bytes=lambda du_lieu, ten, ghi_chu="": _serve_bytes(
            thread_id, thread_type, du_lieu, ten, ghi_chu),
        gui_tep=lambda duong, ten, ghi_chu="": _serve_path(
            thread_id, thread_type, duong, ten, ghi_chu),
        dang_go=lambda: send_typing(thread_id, thread_type),
        gui_tra_loi=lambda chu: _tra_loi_dai_ra_word(thread_id, thread_type, chu),
    )


def _lam_viec_dich(thread_id: str, thread_type: int,
                   pend: dict | None, chon: dict) -> None:
    """Chạy việc người dùng vừa chọn trong menu — xem services/video_giao.py."""
    from services import video_giao as _vg

    _vg.chay(_kenh_zalop(thread_id, thread_type), pend, chon)
