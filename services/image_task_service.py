...
class ImageTaskService:
    ...
    def list_tasks(self, identity: dict[str, object], task_ids: list[str]) -> dict[str, Any]:
        ...
        def _is_media(t: dict) -> bool:
            d = t.get("data")
            if not isinstance(d, list) or not d: return False
            u = str(d[0].get("url") or "").lower()
            return u.endswith((".mp3", ".mp4", ".wav", ".webm"))
        ...
        with self._lock:
            ...
            if not requested_ids:
                items = [
                    ...
                    if task.get("owner_id") == owner and not _is_media(task)
                ]
