...
class OpenCodeProvider:
    ...
    def list_models(self, force_refresh: bool = False) -> list[dict[str, Any]]:
        ...
        try:
            ...
            # Models to forcibly exclude (e.g., they still end in -free but are actually restricted/paid)
            exclude_models = {"minimax-m3-free", "qwen3.6-plus-free"}
            ...
            free_models = [
                ...
                for m in all_models
                if isinstance(m, dict) and str(m.get("id", "")).endswith("-free") and str(m.get("id", "")) not in exclude_models
            ]
