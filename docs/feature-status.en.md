...
| 流式输出支持 | ✅  | 已实现。 |
| 图片尺寸参数 | ✅  | 已实现：`/v1/images/generations` 接受 `size` 参数（`services/protocol/openai_v1_image_generations.py`），默认值来自 `config.default_image_size`。 |
| 服务端图片 URL 缓存 | ✅  | 已实现。 |
| `rt_token` 刷新 | ❌  | 待实现。 |
| 代理配置功能 | ✅  | 已支持网页端配置全局 HTTP / HTTPS / SOCKS5 / SOCKS5H 代理，并应用到出站请求。 |
| Anthropic 协议支持 | ✅  | 已实现：`POST /v1/messages`（`api/ai.py` → `services/protocol/anthropic_v1_messages.py`，SSE 兼容 Anthropic 事件流），附带测试 `test/test_v1_messages.py`。 |
